CREATE OR REPLACE VIEW volume_change.vw_volume_change_latest_observation AS
 SELECT vc.pk_glacier,
   ...
   FROM volume_change.vw_volume_change_latest_observation_date vclod
     LEFT JOIN volume_change.vw_volume_change vc ON vc.pk_glacier = vclod.pk_glacier AND vc.date_to = vclod.latest_observation_date
	 LEFT JOIN volume_change.vw_volume_change_first_observation_date vcfod ON vcfod.pk_glacier = vclod.pk_glacier;
