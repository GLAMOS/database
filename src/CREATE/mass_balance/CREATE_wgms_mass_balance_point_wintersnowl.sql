CREATE OR REPLACE VIEW mass_balance.wgms_mass_balance_point_wintersnow
 AS SELECT
	...
	st_transform(st_setsrid(st_makepoint(mbp.latitude::double precision, mbp.longitude::double precision), 21781), 4326) as geom,	-- to LV03 to WGS84
...
WHERE date_part('year',(mbp.date_to)) = date_part('year', CURRENT_DATE)-1 AND ot.name = 'wintersnow';

ALTER TABLE mass_balance.wgms_mass_balance_point_wintersnow
    OWNER TO gladmin;