...
CREATE OR REPLACE VIEW mass_balance.vw_mb_swisswide
 ...
 SELECT 
    ...
    mbsw.year,
	ROUND(SUM(mbsw.area),6) as area_km2,
	...
	ROUND(SUM(mbsw.vol_evolution),9) as vol_evolution_km3,
	ROUND((SUM(mbsw.vol_evolution)-(LAG(SUM(mbsw.vol_evolution),1) OVER (ORDER by mbsw.year)))*100/(LAG(SUM(mbsw.vol_evolution),1) OVER (ORDER by mbsw.year)),3) as vol_evolution_per
