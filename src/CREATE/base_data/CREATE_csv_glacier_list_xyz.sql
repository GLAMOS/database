...
/*  DBMS       : PostgreSQL                   */
/* ------------------------------------------ */
CREATE OR REPLACE VIEW base_data.csv_glacier_list_xyz AS
	SELECT 
		...
		round((ilxyz.area/1000000::numeric)::numeric, 3)  AS area_km2,
		ilxyz.year_acquisition  AS area_year,
		hlcd.has_length,
	...
	FROM base_data.vw_glacier g
		...
		LEFT JOIN inventory.vw_inventory_latest_xyz ilxyz ON ilxyz.pk_glacier = g.pk
	
	WHERE hlcd.has_length = 1 OR mbohd.mass_balance_observation_has_data = 1 OR hlcd.has_length = 1;
	
ALTER TABLE base_data.csv_glacier_list_xyz
    OWNER TO gladmin;

GRANT ALL ON TABLE base_data.csv_glacier_list_xyz TO gladmin;
GRANT SELECT ON TABLE base_data.csv_glacier_list_xyz TO glporo;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE base_data.csv_glacier_list_xyz TO glrw;
GRANT SELECT ON TABLE base_data.csv_glacier_list_xyz TO glro;