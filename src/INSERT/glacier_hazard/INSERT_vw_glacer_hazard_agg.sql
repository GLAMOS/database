...
SELECT 
	...
	string_agg(distinct ht.hazard_maintype_name,', ') as hazard_main_type,
	string_agg(distinct ht.hazard_subtype_name,', ') as hazard_sub_type,
	string_agg(distinct dt.damage_maintype_name,', ') as damage_main_type,
	string_agg(distinct dt.damage_subtype_name,', ') as damage_sub_type,
	string_agg(distinct dt.damage_detailtype_name,', ') as damage_detail_type,
	e.volume_m3,
	e.area_damaged_m2,
	e.dead_people,
	e.injured_people,
	e.description_de,
	e.description_en,
	string_agg(distinct concat(pub.author,' (',pub.date_of_publication,') ', pub.title),' ;') as primary_source,
	e.secondary_source,
